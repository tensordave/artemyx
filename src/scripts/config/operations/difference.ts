...
export async function executeDifference(
	...
	progressControl.updateProgress(outputId, 'processing', `Differencing ${inputA} minus ${inputB} (${modeLabel})...`);
	...
	if (featureCount === 0) {
		...
		progressControl.updateProgress(outputId, 'success', `No features remaining after difference`);
